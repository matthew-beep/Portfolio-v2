...
export default function Projects({ work }) {
  ...
  const isInView = useInView(ref, {amount: 0.15}); // gonna need to check scroll position too
  const transUp = useInView(ref, {margin:"10px 0px 0px 0px"});
  useEffect(() => {
    console.log(isInView);
  }, [isInView])

  useEffect(() => {
    console.log("100 px away from top " + transUp);
  }, [transUp])
  // need to add nav bar outside of main
  return (
    <section id="projects" className='min-h-screen h-auto z-20 flex justify-center items-start pt-64 pb-20 relative text-white sticky top-0 border-2'>
      <div ref={ref} id="file"className='flex flex-col gap-2 project-section bg-[#161B22] w-11/12 md:w-9/12 h-auto rounded-lg md:py-10 md:px-10 py-5 border-2'>
